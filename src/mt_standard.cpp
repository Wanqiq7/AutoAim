...
int main(int argc, char * argv[])
{
  ...
  std::atomic<io::GimbalMode> mode{gimbal.mode()};
  auto last_mode{mode.load()};
  ...
  while (!exiter.exit()) {
    ...
    /// 打符
      ...
      if (mode.load() == io::GimbalMode::SMALL_BUFF) {
        ...
        buff_command = buff_aimer.aim(target_copy, t, state.bullet_speed, true);
      } else if (mode.load() == io::GimbalMode::BIG_BUFF) {
        buff_big_target.get_target(power_runes, t);
        auto target_copy = buff_big_target;
        buff_command = buff_aimer.aim(target_copy, t, state.bullet_speed, true);
      }
      float yaw_deg = static_cast<float>(buff_command.yaw * 57.2957795);
      float pitch_deg = static_cast<float>(buff_command.pitch * 57.2957795);
      gimbal.send(buff_command.control, buff_command.shoot, yaw_deg, 0, 0, pitch_deg, 0, 0);
